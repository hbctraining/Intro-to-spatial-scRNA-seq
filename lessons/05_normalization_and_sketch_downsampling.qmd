...
It returns a Seurat object with a new assay (sketch), consisting of `ncells` bins selected based off a **"leverage score"** for each bin.
...
```{r}
VlnPlot(seurat_sketch, "leverage.score")
```
