...
title: "Clustering"
description: |
  Use graph-based clustering methods k-nearest neighbors and Leiden to define clusters based upon gene expression scores for single-cell datasets. You will examine how parameters like resolution affect cluster size and tissue interpretation.
author:
  - Noor Sohail
date: "2025-07-22"
categories: 
...
At this point in the analysis, we have succesfully calculated PC scores, creating meaningful embeddings of our bins. Using this principal component, we can now evaluate the similarity of bins through clustering. This is a two-step process where we first calcualte k-nearest neighbors to build a graph structure, linking similar bins together. Then, we will utilize the Leiden algorithm to cluster bins into larger groups. These cluster labels will be reused to identify spatial domains and major celltypes in the dataset.
...
Another key parameter is `resolution`, which controls the **granularity of the clustering** and must be tuned for each experiment. **Higher resolution values produce more clusters**. For a first-pass look at the data, a lower resolution is sufficient to broad patterns and communities. Typically, we recommend that datasets on the order of 30,000–50,000 cells/bins, setting a `resolution` between `0.4` and `1.4` typically yields reasonable clustering. So for a much larger dataset, such as this one, we will be setting a low resolution parameter to start.
...
In the ideal scenario, we expect that the clusters should correspond with the celltypes in our dataset at this point. When we began the analysis, we identified the celltypes that we expect to see in the dataset. 
...
#| fig-cap: Violin plot of `PECAM1` and `VWF` for each cluster to potentially identify populations of Endothelial cells.
