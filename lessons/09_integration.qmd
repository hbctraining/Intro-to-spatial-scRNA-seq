...
_Image source: [Intro to scRNA-seq](https://hbctraining.github.io/Intro-to-scRNAseq/lessons/08_integration_cca_theory.html)_
::: 
...
Notably, we see that that clusters 2 and 12 contain almost exclusively `P5CRC` bins. So let us zoom in on these bins to see if there is a biological explanation for this observation, or if it is a technical artifact that we need to correct for with integration.

::: {.callout-note collapse=true}
# Zoom in on clusters of interest with `DimPlot()`

First, we will identify which bins belong to clusters 1 and 12 with the `WhichCells()` function. We will supply that output to the `cells.highlight` argument in `DimPlot()` to highlight these bins on the UMAP.

```{r}
#| label: fig-highlight_clusters
#| fig-cap: Highlight clusters 2 and 12 on the UMAP.
...
Now bins that have the label of cluster 2 or 12 are colored red while the rest are gray in the UMAP.
:::
...
```{r}
#| label: fig-ceacam5_featureplot
#| fig-cap: CEACAM5 expression plotted on the UMAP.
...
```{r}
#| label: fig-ceacam5_vlnplot
#| fig-cap: CEACAM5 expression plotted as a violin plot for each cluster.
...
```{r}
#| label: fig-ceacam6_featureplot
#| fig-cap: CEACAM6 expression plotted on the UMAP.
FeaturePlot(seurat_processed, 
            features = "CEACAM6",
            label = TRUE)
```
...
```{r}
#| label: fig-ceacam6_vlnplot
#| fig-cap: CEACAM6 expression plotted as a violin plot for each cluster.
VlnPlot(seurat_processed, 
        features = "CEACAM6", 
        pt.size = 0) +
  NoLegend()
...
We see that clusters 2 and 12 do indeed have higher expression of the tumor marker genes! 
...
From this, we have the additional knowledge that there are likely some tumor cells among clusters 1 and 9.
...
_Image Source: [Korsunsky et al., 2019](https://www.nature.com/articles/s41592-019-0619-0)
:::
