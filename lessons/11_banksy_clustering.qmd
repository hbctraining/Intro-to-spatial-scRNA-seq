...
To start, we are going to open a new script and save it as `06_BANKSY.R` to our `scripts` directory. Then, we will load all the libraries necessary for this analysis as usual.
...
We already have the `group` parameter with `orig.ident` (representing our two samples) and so we will set `split.scale` to `True`. Therefore, the next parameter we need to supply in the `@meta.data` are the `dimx` and `dimy` spatial coordinates for every bin in the dataset.
...
Next, we are going to add these coordinates into our metadata, so that we can directly call this column from the `RunBanksy()` function.
...
# Inspect the metadata with the x and y coordinates added 
...
The `RunBanksy()` step generated a **new counts matrix** from the original counts matrix. In practice, what happens is that BANKSY will concatenate 3 different matrices together, all of which are computed from the top highly variable features.
...
Features(seurat_banksy)[2001:2006]

# Last features in BANKSY matrix
Features(seurat_banksy)[4001:4006]
```
...
#| fig-width: 10
#| fig-height: 10
# Visualize the spatial overlay of both BANKSY derived clusters and gene expression derived clusters for both samples
...
For one, we can see that there is less "noise" in the `seurat_banksy` clusters, where bins of a cluster are not as scattered throughout the slide. This is what would be expected from the BANKSY algorithm.
...
Just like we saw in the [clustering lesson](08_clustering.qmd), we can once again answer the question of how well these BANKSY cluster align with the expected celltypes.
...
# Create violin plots for two Smooth Muslce cells marker genes
