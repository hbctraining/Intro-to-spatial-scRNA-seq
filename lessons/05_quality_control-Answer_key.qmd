...
1. Do you notice a pattern in cells in regards to the number of UMIs and feature? Make a `geom_point` plot to compare these values on a per-cell basis and color each point by the mitochondrial ratio following the structure provided here:
...
We can calculate the difference in the number of columns (bin) to see how many were removed after our filtration step.

```{r}
#| label: nbins_filtered
# Difference in number of bins
ncol(seurat_merged) - ncol(seurat_filtered)
```

We can also create a barplot to directly compare against the first visual made in the lesson too!
