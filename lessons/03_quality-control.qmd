...
seurat_merged <- readRDS("data/seurat_merged.RDS")
```
...
ggplot(meta) +
  geom_density(aes(x = nCount_Spatial.016um, fill = orig.ident),
               alpha = 0.4,
               color = "black") +
...
meta_filt <- subset(meta,
  ((orig.ident == "P5CRC") & (nCount_Spatial.016um > 30)) |
  ((orig.ident == "P5NAT") & (nCount_Spatial.016um > 50)))

ggplot(meta_filt) +
  geom_density(aes(x = nCount_Spatial.016um,
                     fill = orig.ident),
                 alpha = 0.4,
                 color = "black") +
  geom_vline(xintercept = 30, color = "pink") +
...
ggplot(meta) +
  geom_density(aes(x = nFeature_Spatial.016um,
                     fill = orig.ident),
                 alpha = 0.4,
                 color = "black") +
  geom_vline(xintercept = 30, color = "pink") +
...
meta_filt <- subset(meta,
  ((orig.ident == "P5CRC") & (nFeature_Spatial.016um > 30)) |
  ((orig.ident == "P5NAT") & (nFeature_Spatial.016um > 50)))
...
seurat_merged$log10GenesPerUMI <- log10(seurat_merged$nFeature_Spatial.016um) / 
                                  log10(seurat_merged$nCount_Spatial.016um)
```
...
SpatialFeaturePlot(seurat_filtered, 
                   c("nFeature_Spatial.016um", "nCount_Spatial.016um"),
                   pt.size.factor = 13,
                   image.alpha = 0)
```
