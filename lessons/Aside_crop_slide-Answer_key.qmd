...
1. The area not contain colorectal samples comes from another place on the same slide. Using a loop and the parameter below, subset the non-cancer region of the slide for 8μm and 16μm bin sizes.
...
First, we can download the data and uncompress it. Note, this will overwrite our `binned_outputs` directory from the colorectal cancer sample.
...
```{r}
#| label: overwrite_uncompressed_binned_outputs
#| echo: false
# Set URL for where to download the data from
URL <- "https://cf.10xgenomics.com/samples/spatial-exp/3.0.0/Visium_HD_Human_Colon_Normal_P5/Visium_HD_Human_Colon_Normal_P5_binned_outputs.tar.gz"
...
[Back to Lesson >>](04_theory_of_PCA.qmd)
