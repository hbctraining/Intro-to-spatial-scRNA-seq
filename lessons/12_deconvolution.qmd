...
At this point, we have assigned labels for each bin in the sketch assay. So we can once again run `ProjectData()` and this time get celltype labels for each bin in our dataset.
...
As we might expect, the immune populations (myeloid, T and B cells) appear close to one another in UMAP space due to the similarity of their gene expression. The most distinctly clustered populations are the tumor and epithelial bins, which are do not have as many similarities as the rest of the cell types in the dataset.
