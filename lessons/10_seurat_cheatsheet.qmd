...
all(rownames(seurat_banksy@meta.data) == Cells(seurat_banksy))
```
...
seurat_banksy <- RenameIdents(object = seurat_banksy, 
                                 "1" = "CD14+ monocytes",
                                 "3" = "CD14+ monocytes",
                                 "2" = "Activated T cells")
...
seurat_banksy$celltype <- Idents(seurat_banksy)
```
...
VariableFeatures(seurat_banksy) %>% head()
```
...
DefaultAssay(seurat_banksy) <- "sketch"
...
LayerData(seurat_banksy, assay="Spatial.008um", layer="counts")[1:5, 1:5]
```
...
FetchData(seurat_banksy, vars=c("PTPRC", "sample"), 
...
FetchData(seurat_banksy, vars=c("sketch_PTPRC", "Spatial.008um_PTPRC"), 
          layer="data") %>% head()
```
...
# seurat_banksy[['pca']]@cell.embeddings
Embeddings(seurat_banksy, reduction="pca.sketch")[1:5, 1:5]
```
...
Loadings(seurat_banksy, reduction="pca.sketch")[1:5, 1:5]
```
...
ProjectDim(seurat_banksy, reduction = "pca.sketch")
```
...
# seurat_banksy[['umap']]@cell.embeddings
Embeddings(seurat_banksy, reduction="umap.sketch") %>% head()
```
...
DimPlot(seurat_banksy) + ggtitle("Seurat clusters")
```
...
DimPlot(seurat_banksy, group.by = "orig.ident")
```
...
DimPlot(seurat_banksy, split.by = "orig.ident", group.by="orig.ident")
```
...
FeatureScatter(seurat_banksy, feature1 = "MT-ND5", feature2 = "mitoRatio") + 
...
cell1 <- Cells(seurat_banksy)[1] 
cell2 <- Cells(seurat_banksy)[2]
...
seurat_banksy@meta.data %>% 
  subset(rownames(seurat_banksy@meta.data) %in% c(cell1, cell2)) %>% 
  select(orig.ident, nCount_Spatial.008um)
```

```{r}
#| label: cellscatter
CellScatter(seurat_banksy, cell1=cell1, cell2=cell2)
```
...
```{r fig.width=9}
#| label: vlnplot
VlnPlot(seurat_banksy, c("CD14", "CD79A"))
```
...
VlnPlot(seurat_banksy, c("CEACAM6", "CEACAM5", "CD52", "CXCL8"), 
...
```{r}
#| label: ridgeplot
RidgePlot(seurat_banksy, "VIM", assay="Spatial.008um") + NoLegend()
```
...
```{r}
#| label: dimheatmap
DimHeatmap(seurat_banksy, nfeatures = 10, reduction = "pca.sketch")
```
...
```{r fig.width=8}
#| label: doheatmap
DoHeatmap(seurat_banksy, features=c("CD14", "FCGR3A", "FCER1A", 
                                        "IL3RA", "CD79A", "CD3D"))
```
...
DotPlot(seurat_banksy, markers, assay="Spatial.008um")
```
***
