...
description: |
  Keep a compact Seurat cheatsheet at hand that summarizes key functions for spatial transcriptomics workflows, from data loading to clustering and plotting. Use this reference to speed up scripting, avoid common syntax errors, and standardize your analysis pipeline.
author:
  - Noor Sohail
  - Meeta Mistry
  - Will Gammerdinger
...
Here, you’ve already used Seurat through several steps and seen many function calls. In this lesson you consolidate the core Seurat commands and object operations for spatial analysis into a concise reference. It doesn’t introduce new analysis concepts - instead, it makes it easier to reproduce and script what you’ve learned without constantly searching documentation. This quick reference speeds up all later work when you adapt the pipeline to your own data.
...
```{r}
#| label: show_colnames
colnames(seurat_processed) %>% head()
```
...
```{r}
#| label: show_features
Features(seurat_processed) %>% head()
```
...
```{r}
#| label: show_rownames
rownames(seurat_processed) %>% head()
```
...
VariableFeatures(seurat_processed) %>% head()
```
...
LayerData(seurat_processed, assay="Spatial.008um", layer="counts")[1:5, 1:5]
```
...
FetchData(seurat_processed, vars=c("PTPRC", "sample"), 
          assay="sketch", layer="data") %>% head()
```
...
FetchData(seurat_processed, vars=c("sketch_PTPRC", "Spatial.008um_PTPRC"), 
          layer="data") %>% head()
```
...
Embeddings(seurat_processed, reduction="pca.sketch")[1:5, 1:5]
```
...
Loadings(seurat_processed, reduction="pca.sketch")[1:5, 1:5]
```
...
ProjectDim(seurat_processed, reduction = "pca.sketch")
```
...
Embeddings(seurat_processed, reduction="umap.sketch") %>% head()
```
...
DimPlot(seurat_processed) + ggtitle("Seurat clusters")
```
...
DimPlot(seurat_processed, group.by = "orig.ident")
```
...
DimPlot(seurat_processed, split.by = "orig.ident", group.by="orig.ident")
```
...
Idents(seurat_processed) <- "seurat_cluster.projected"
FeatureScatter(seurat_processed, feature1 = "MT-ND5", feature2 = "mitoRatio") + 
...
cell1 <- Cells(seurat_processed)[1] 
cell2 <- Cells(seurat_processed)[2]
...
seurat_processed@meta.data %>% 
  subset(rownames(seurat_processed@meta.data) %in% c(cell1, cell2)) %>% 
  select(orig.ident, nCount_Spatial.008um)
```

```{r}
#| label: cellscatter
CellScatter(seurat_processed, cell1=cell1, cell2=cell2)
```
...
```{r fig.width=9}
#| label: vlnplot
VlnPlot(seurat_processed, c("CD14", "CD79A"))
```
...
VlnPlot(seurat_processed, c("CEACAM6", "CEACAM5", "CD52", "CXCL8"), 
        group.by="orig.ident", ncol=2, pt.size=0)
```
...
```{r}
#| label: ridgeplot
RidgePlot(seurat_processed, "VIM", assay="Spatial.008um") + NoLegend()
```
...
```{r}
#| label: dimheatmap
DimHeatmap(seurat_processed, nfeatures = 10, reduction = "pca.sketch")
```
...
```{r fig.width=8}
#| label: doheatmap
DoHeatmap(seurat_processed, features=c("CD14", "FCGR3A", "FCER1A", 
                                        "IL3RA", "CD79A", "CD3D"))
```
...
DotPlot(seurat_processed, markers, assay="Spatial.008um")
```
***
