...
```{r}
#| label: get_tissue_coordinates_1
...
```{r}
#| label: get_tissue_coordinates_2
...
3. What are the 5 least variable genes (of the top 3,000) in the Seurat object?
:::
...
5. Show the code to get the entire sketch log-normalized (`data`) count matrix.
:::
...
6. Show how you would use the `FetchData()` function to generate a dataframe of `fullumapsketch_1`, `fullumapsketch_2` and `orig.ident` values for each cell.
:::
...
```{r}
#| label: fig-dimplot_ggtitle
#| fig-cap: "`DimPlot` highlighting the the different clusters on the UMAP."
...
```{r}
#| label: fig-dimplot_groupby
#| fig-cap: "Grouping the `DimPlot` by the `orig.ident`."
...
```{r}
#| label: fig-dimplot_splitby
#| fig.width: 8
#| fig-cap: "Plot the `DimPlot` side-by-side for the two `orig.ident` and color them likewise."
...
```{r}
#| label: fig-spatialdimplot
#| fig-cap: "Grouping the `SpatialDimPlot` by the `orig.ident`."
#| fig-width: 10
...
There are a [variety of different parameters](https://satijalab.org/seurat/reference/spatialplot) that can be specified in order to create the plot that you want to make.
...
#| fig.width: 8
#| fig-cap: "Displaying the expression of two Endothelial cells markers on the UMAP."
...
```{r}
#| label: fig-featureplot_cutoff
#| fig-width: 8
#| fig-cap: "Displaying the expression of two Endothelial cells markers on the UMAP, while applying a `min.off`."
...
```{r}
#| label: fig-featureplot_labelcluster
#| fig-cap: "Displaying the expression of a Tumor cell marker LCN2 on the UMAP, while also showing the cluster labels."
...
#| fig-cap: "Displaying the expression of a Tumor cell marker LCN2 on the tissue."
#| fig-width: 10
...
```{r}
#| label: fig-featurescatter
#| fig-cap: "A scatterplot showing the relation between two features (MitoRatio and MT-ND5 expression) across the dataset."
# Set idents to seurat_cluster.projected
Idents(seurat_processed) <- "seurat_cluster.projected"
...
```{r}
#| label: fig-cellscatter
#| fig-cap: "A scatterplot showing the relationship in expression between two bins."
...
```{r}
#| label: fig-vlnplot
#| fig-cap: "Violin plots showing the distribution of two Tumor cell markers across the different clusters."
#| fig-width: 9
...
```{r}
#| label: fig-vlnplot_customization
#| fig-cap: "Violin plots showing the distribution of four Tumor cell markers when comparing the `orig.ident`.`"
...
```{r}
#| label: fig-ridgeplot
#| fig-cap: "A `RidgePlot` showing the distribution of a Tumor marker across the different clusters."
...
```{r}
#| label: fig-dimheatmap
#| fig-cap: "A heatmap showing the top and bottom most influential genes on PC1."
...
```{r}
#| label: fig-dimheatmap_PC2
#| fig-cap: "A heatmap showing the top and bottom most influential genes on PC2."
...
```{r}
#| label: fig-dimheatmap_PC13
#| fig-cap: "A heatmap showing the top and bottom most influential genes on PC1, PC2 and PC3."
...
```{r}
#| label: fig-doheatmap
#| fig-cap: "A heatmap showing expression of three Tumor cell markers across the different clusters."
#| fig-width: 8
...
```{r}
#| label: fig-dotplot
#| fig-cap: "A `DotPlot` displaying various celltypes and their marker genes."
#| fig-width: 15
#| fig-height: 9
