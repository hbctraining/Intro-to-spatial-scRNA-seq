...
```{r}
#| label: overwite_uncompressed_binned_outputs
#| echo: false
# Set URL for where to download the data from
URL <- "https://cf.10xgenomics.com/samples/spatial-exp/3.0.0/Visium_HD_Human_Colon_Cancer_P5/Visium_HD_Human_Colon_Cancer_P5_binned_outputs.tar.gz"
...
We can see that this object has **`r format(nrow(seurat_obj), scientific = FALSE)`** genes **`r format(ncol(seurat_obj), scientific = FALSE)`** bins.
...
lowres_image <- image_read("data/binned_outputs/square_016um/spatial/tissue_lowres_image.png") %>% 
  image_flip %>% 
  as.raster(interpolate = TRUE) %>% 
  rasterGrob()

# Read in and orient the detected tissue image
detected_image <- image_read("data/binned_outputs/square_016um/spatial/detected_tissue_image.jpg") %>% 
  as.raster(interpolate = TRUE) %>% 
  rasterGrob()
...
# Read parquet file
parquet_df <- read_parquet("data/binned_outputs/square_016um/spatial/tissue_positions.parquet")
```
...
path_json <- paste0("data/binned_outputs/square_016um/spatial/scalefactors_json.json")
# Set path for Parquest file
path_parq <- paste0("data/binned_outputs/square_016um/spatial/tissue_positions.parquet")
# Set path for PNG file
path_png  <- paste0("data/binned_outputs/square_016um/spatial/tissue_lowres_image.png")
...
for (bin in bins) {
  ...
  # Set path for JSON file
  path_json <- paste0("data/binned_outputs/square_", bin, "um/spatial/scalefactors_json.json")
  # Set path for Parquet file
  path_parq <- paste0("data/binned_outputs/square_", bin, "um/spatial/tissue_positions.parquet")
  # Set path for PNG file
  path_png  <- paste0("data/binned_outputs/square_", bin, "um/spatial/tissue_lowres_image.png")
