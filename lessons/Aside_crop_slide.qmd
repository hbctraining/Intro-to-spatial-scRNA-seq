...
2.  In a single 6.5mm x 6.5mm capture area from Visium HD, we would expect that to correspond to about 20,000 pixels by 20,000 pixels. Do our expectations match our observed data?

3.  By comparing the dimensions of `coordinates`, which only includes coordinates for locations that Space Ranger detected tissue, and `exercise_coordinates`, which contains all coordinates with and without tissue detected by Space Ranger, how many pixels didn't have tissue detected in them?
